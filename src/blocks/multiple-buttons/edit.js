...
class MultipleButtonsEdit extends Component {
	render() {
		...
		const getTemplate = memoize( ( number ) => {
			return times( number, () => [ 'gt-blocks-pro/button', buttonAttributes || {} ] );
		} );
			...
			<Fragment>
				...
				<InspectorControls key="inspector">
					...
					<PanelBody title={ __( 'Layout Settings', 'gt-blocks-pro' ) } initialOpen={ true } className="gt-panel-layout-settings gt-panel">
						...
						<RangeControl
							label={ __( 'Number of buttons', 'gt-blocks-pro' ) }
							value={ buttons }
						...
						<BaseControl id="gt-alignment-control" label={ __( 'Alignment', 'gt-blocks-pro' ) }>
							<AlignmentToolbar
								value={ alignment }
								onChange={ ( newAlignment ) => setAttributes( { alignment: newAlignment } ) }
							/>
