...
class GridEdit extends Component {
	...
	render() {
		...
		const getItemsTemplate = memoize( ( number ) => {
			const block = [ 'gt-blocks-pro/column', {
				allowedBlocks: allowedBlocks,
				template: template,
				templateLock: templateLock || false,
			} ];
			...
			<Fragment>
					...
					<Toolbar
						controls={
							[ 2, 3, 4 ].map( column => ( {
								...
								title: sprintf( __( '%s Columns', 'gt-blocks-pro' ), column ),
				...
				<InspectorControls key="inspector">
					...
					<PanelBody title={ __( 'Layout Settings', 'gt-blocks-pro' ) } initialOpen={ true } className="gt-panel-layout-settings gt-panel">
						...
						<RangeControl
							label={ __( 'Columns', 'gt-blocks-pro' ) }
							value={ columns }
						...
						<BaseControl label={ __( 'Add grid item', 'gt-blocks-pro' ) }>
							<Button
								isLarge
				...
				<div id={ blockId } className={ className }>
					...
					{ ( isSelected || isChildBlockSelected ) && (
						...
						>
							...
							{ __( 'Change grid layout', 'gt-blocks-pro' ) }
						</Button>
					) }
				</div>
			</Fragment>
