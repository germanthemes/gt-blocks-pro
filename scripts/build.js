...
const fileBuildJS = path.resolve( theCWD, './assets/js/gt-blocks-pro-editor.js' );
const fileEditorCSS = path.resolve( theCWD, './assets/css/gt-blocks-pro-editor.css' );
const fileStyleCSS = path.resolve( theCWD, './assets/css/gt-blocks-pro.css' );
...
async function build( webpackConfig ) {
	...
	compiler.run( ( err, stats ) => {
		...
		console.log(
			...
			`${ chalk.dim( '— ./assets/js/' ) }`,
			`${ chalk.green( 'gt-blocks-pro-editor.js' ) }`,
			'\n',
			getFileSize( fileEditorCSS ),
			`${ chalk.dim( '— ./assets/css/' ) }`,
			`${ chalk.green( 'gt-blocks-pro-editor.css' ) }`,

			'\n',
			getFileSize( fileStyleCSS ),
			`${ chalk.dim( '— ./assets/css/' ) }`,
			`${ chalk.green( 'gt-blocks-pro.css' ) }`,
